...
Run the tests: `pytest -q` (25 tests).
...
```
                              START
                                │
                          ┌─────▼─────┐
                          │  safety   │  deterministic triage + LLM review
                          └─────┬─────┘  (LLM may escalate, never downgrade)
                    blocked ────┴──── proceed
                       │               │
                ┌──────▼──────┐  ┌─────▼─────┐
                │ blocked_exit│  │  routing  │  request → department
                └──────┬──────┘  └─────┬─────┘
                       │        confidence < 0.65
                       │               │
                       │      ┌────────▼────────┐
                       ...
                       │      └────┬───────┬────┘     state checkpointed,
                       │  approved │       │ rejected  process may exit
                       │           │       │
                       │           │  ┌────▼─────────┐
                       │           │  │rejected_exit │
                       │           │  └────┬─────────┘
                       │     ┌─────▼───────┴──┐
                       │     │  appointment   │  slots → conflicts → book
                       │     └───────┬────────┘
                       │     ┌───────▼────────┐
                       │     │   document     │  checksum → classify → gaps
                       │     └───────┬────────┘
                       │     ┌───────▼────────┐
                       │     │   followup     │  reminders + follow-up tasks
                       │     └───────┬────────┘
                       │     ┌───────▼────────┐
                       │     │  coordinator   │  confirmation from persisted facts,
                       │     └───────┬────────┘  passed through the output guard
                       └─────────────┴──► END
```
...
| `appointment` | pure logic (no LLM) | `find_available_slots`, `book_appointment`, `reschedule_appointment`, `cancel_appointment` | `department`, `request_text` → `appointment`, `timeframe`, `available_slots` |
